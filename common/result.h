...
EXPORTED void DieWithMessage(const std::string &msg);

EXPORTED void InvalidValueOrDie(const Status &st);
...
/// ```
template <class T>
class AVA_MUST_USE_TYPE Result : public util::EqualityComparable<Result<T>> {
  template <typename U>
  friend class Result;
 ...
 public:
  ...
  Result(const Status &status)  // NOLINT(runtime/explicit)
      : status_(status) {
    if (__AVA_PREDICT_FALSE(status.ok())) {
      internal::DieWithMessage(std::string("Constructed with a non-error status: ") + status.ToString());
    }
  }
  ...
  template <typename U, typename E = typename std::enable_if<
                            ...
                            !std::is_same<typename std::remove_reference<typename std::remove_cv<U>::type>::type,
                                          Status>::value>::type>
  ...
  Result(const Result &other) : status_(other.status_) {
    if (__AVA_PREDICT_TRUE(status_.ok())) {
  ...
  ///
  /// \param other The value to copy from.
  template <typename U, typename E = typename std::enable_if<std::is_constructible<T, const U &>::value &&
  ...
  template <typename U, typename E = typename std::enable_if<std::is_constructible<T, U &&>::value &&
                                                             std::is_convertible<U, T>::value>::type>
  Result(Result<U> &&other) noexcept {
    if (__AVA_PREDICT_TRUE(other.status_.ok())) {
  ...
  /// the stored error.
  template <typename M>
  typename EnsureResult<typename std::result_of<M && (const T &)>::type>::type Map(M &&m) const & {
    if (!ok()) {
      return status();
    }
  ...
  /// Cast the internally stored value to produce a new result or propagate the stored
  /// error.
  template <typename U, typename E = typename std::enable_if<std::is_constructible<U, const T &>::value>::type>
  Result<U> As() const & {
    if (!ok()) {
      return status();
    }
  ...
  const T &ValueUnsafe() const & { return *internal::launder(reinterpret_cast<const T *>(&data_)); }

  T &ValueUnsafe() & { return *internal::launder(reinterpret_cast<T *>(&data_)); }

  T ValueUnsafe() && { return MoveValueUnsafe(); }

  T MoveValueUnsafe() { return std::move(*internal::launder(reinterpret_cast<T *>(&data_))); }
 ...
 private:
  ...
  void Destroy() {
    if (__AVA_PREDICT_TRUE(status_.ok())) {
      static_assert(offsetof(Result<T>, status_) == 0, "Status is guaranteed to be at the start of Result<>");
      internal::launder(reinterpret_cast<const T *>(&data_))->~T();
    }
  }
};

#define AVA_ASSIGN_OR_RAISE_IMPL(result_name, lhs, rexpr)                            \
  auto &&result_name = (rexpr);                                                      \
  AVA_RETURN_IF_(!(result_name).ok(), (result_name).status(), AVA_STRINGIFY(rexpr)); \
  lhs = std::move(result_name).ValueUnsafe();

#define AVA_ASSIGN_OR_RAISE_NAME(x, y) AVA_CONCAT(x, y)
...
///   AVA_ASSIGN_OR_RAISE(auto value, MaybeGetValue(arg));
///
/// Example: Assigning to an existing value:
///   ValueType value;
///   AVA_ASSIGN_OR_RAISE(value, MaybeGetValue(arg));
///
/// WARNING: AVA_ASSIGN_OR_RAISE expands into multiple statements;
/// it cannot be used in a single statement (e.g. as the body of an if
/// statement without {})!
///
/// WARNING: AVA_ASSIGN_OR_RAISE `std::move`s its right operand. If you have
/// an lvalue Result which you *don't* want to move out of cast appropriately.
#define AVA_ASSIGN_OR_RAISE(lhs, rexpr) \
  AVA_ASSIGN_OR_RAISE_IMPL(AVA_ASSIGN_OR_RAISE_NAME(_error_or_value, __COUNTER__), lhs, rexpr);
