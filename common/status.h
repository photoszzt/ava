...
#define AVA_RETURN_IF_(condition, status, _) \
  do {                                       \
    if (__AVA_PREDICT_FALSE(condition)) {    \
      return (status);                       \
    }                                        \
  } while (0)

#define AVA_RETURN_IF(condition, status) AVA_RETURN_IF_(condition, status, AVA_STRINGIFY(status))

/// \brief Propagate any non-successful Status to the caller
#define AVA_RETURN_NOT_OK(status)                                 \
  do {                                                            \
    ::ava::Status __s = ::ava::internal::GenericToStatus(status); \
    AVA_RETURN_IF_(!__s.ok(), __s, AVA_STRINGIFY(status));        \
  } while (false)

#define RETURN_NOT_OK_ELSE(s, else_)                        \
  do {                                                      \
    ::ava::Status _s = ::ava::internal::GenericToStatus(s); \
    if (!_s.ok()) {                                         \
      else_;                                                \
      return _s;                                            \
    }                                                       \
...
#define RETURN_NOT_OK(s) AVA_RETURN_NOT_OK(s)
#endif
...
/// attached.
class AVA_MUST_USE_TYPE EXPORTED Status : public ava::util::EqualityComparable<Status>,
                                          public ava::util::ToStringOstreamable<Status> {
 public:
  // Create a success status.
  Status() noexcept : state_(nullptr) {}
  ~Status() noexcept {
    if (__AVA_PREDICT_FALSE(state_ != NULL)) {
      DeleteState();
    }
  }
  ...
  static Status FromArgs(StatusCode code, Args &&... args) {
    return Status(code, ava::util::StringBuilder(std::forward<Args>(args)...));
  }

  template <typename... Args>
  static Status FromDetailAndArgs(StatusCode code, std::shared_ptr<StatusDetail> detail, Args &&... args) {
    return Status(code, ava::util::StringBuilder(std::forward<Args>(args)...), std::move(detail));
  }
...
std::ostream &operator<<(std::ostream &os, const StatusCode &sc);
